#!/usr/bin/env bash
# Agent Letterbox for Herdr installer.
set -euo pipefail

repo="${AGENT_LETTERBOX_REPO:-https://github.com/SimonMallas/agent-letterbox-herdr.git}"
install_dir="${AGENT_LETTERBOX_INSTALL_DIR:-$HOME/.local/share/agent-letterbox-herdr}"
bin_dir="${AGENT_LETTERBOX_BIN_DIR:-$HOME/.local/bin}"

fail() { printf 'agent-letterbox-herdr installer: %s\n' "$*" >&2; exit 1; }
...
if [[ -d "$install_dir/.git" ]]; then
  printf 'Updating Agent Letterbox for Herdr in %s\n' "$install_dir"
  git -C "$install_dir" pull --ff-only
else
  printf 'Downloading Agent Letterbox for Herdr...\n'
...
printf '\nInstalled Agent Letterbox for Herdr.\n'
